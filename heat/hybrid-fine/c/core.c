...
void exchange_init(field *temperature, parallel_data *parallel)
{
    ...
    MPI_Irecv(temperature->data[temperature->nx + 1], temperature->ny + 2, 
        MPI_DOUBLE, parallel->ndown, 11, MPI_COMM_WORLD, &parallel->requests[1]);
    ...
    MPI_Irecv(temperature->data[0], temperature->ny + 2, 
        MPI_DOUBLE, parallel->nup, 12, MPI_COMM_WORLD, &parallel->requests[3]);
}
